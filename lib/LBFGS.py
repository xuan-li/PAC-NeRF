...
class LBFGS(Optimizer):
    ...
    def _gather_flat_grad(self):
        ...
        for p in self._params:
            if p.grad is None:
                view = p.data.new(p.data.numel()).zero_().cpu()
            elif p.grad.data.is_sparse:
                view = p.grad.data.to_dense().view(-1).cpu()
            else:
    ...
    def _add_update(self, step_size, update):
        ...
        for p in self._params:
            ...
            p.data.add_(step_size, update[offset:offset + numel].view_as(p.data).to(p.device))
            offset += numel
    ...
    def _copy_params(self):
        ...
        for param in self._params:
            current_params.append(deepcopy(param.data.cpu()))
        return current_params

    def _load_params(self, current_params):
        i = 0
        for param in self._params:
            param.data[:] = current_params[i].to(param.device)
            i += 1
    ...
    def two_loop_recursion(self, vec):
        ...
        for i in range(num_old):
            if old_stps[i].dot(old_dirs[i]) > 1e-8:
    ...
    def _step(self, backward_fn, p_k, g_Ok, g_Sk=None, options=None):
        """
        ...
        if line_search == 'Armijo':
            ...
            return F_new, g_new, t, d, ls_step, closure_eval, 1, desc_dir, fail
        ...
        # perform weak Wolfe line search
            ...
            return F_new, g_new, t, d, ls_step, closure_eval, grad_eval, desc_dir, fail
    ...
    def step(self, backward_fn, p_k, g_Ok, g_Sk=None, options={}):
        return self._step(backward_fn, p_k, g_Ok, g_Sk, options)
...
class FullBatchLBFGS(LBFGS):
    ...
    def step(self, backward_fn, options=None):
        """
        Performs a single optimization step.
        ...
        # take step
        return self._step(backward_fn, p, grad, options=options)
